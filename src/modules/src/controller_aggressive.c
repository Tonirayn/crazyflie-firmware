                                         ...
                                         const uint32_t tick)
{
  ...
  // update integral errors for mellinger
  ...
  current_thrust_a = vdot(target_thrust, z_axis) + vdot(mkvec(gain_setpoint.Gain1[0],gain_setpoint.Gain1[1],gain_setpoint.Gain1[2]),r_error) \
  			...
  			+ vdot(mkvec(gain_setpoint.Gain1[6],gain_setpoint.Gain1[7],gain_setpoint.Gain1[8]),eR) \
   ...
   x = vdot(mkvec(gain_setpoint.Gain ## i[0],gain_setpoint.Gain ## i[1],gain_setpoint.Gain ## i[2]),r_error) \
			...
			+ vdot(mkvec(gain_setpoint.Gain ## i[6],gain_setpoint.Gain ## i[7],gain_setpoint.Gain ## i[8]),eR) \
   ...
   control->thrust = kf*current_thrust_a;

   control->roll = clamp(kf_l*M.x, -32000, 32000);
   control->pitch = clamp(kf_l*M.y, -32000, 32000);
   control->yaw = clamp(km*M.z, -32000, 32000);
